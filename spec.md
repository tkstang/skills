...
| R2 | Discover Codex, Claude, and Cursor from an explicit canonical source worktree with bounded, zero-persistence reads and optional sanitized previews. Guidance-only Claude/Codex discovery retains candidates whose exact metadata and cwd are extracted from a bounded prefix even when the file or record stream continues, summarizes unrelated missing, malformed, unreadable, or unresolvable cwd evidence by provider and stable path-free reason, and scopes preview/prepare discovery to the provider in the exact qualified key; enumeration, aggregate-budget, and deadline failures remain incomplete. The paused executor and other locator consumers remain strict all-or-error. Separate Cursor IDE/CLI origin and launchability; incomplete discovery or ambiguous identity is explicit. | prev1-t01, prev1-t02, prev1-t07, prev1-t12 |
