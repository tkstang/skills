...
async function candidateDerivedFieldsBounded(runtime3, transcriptPath, signature, cache, budget, diagnostic, unattributablePolicy = "fail", unattributable) {
  ...
  if (runtime3 === "claude-code") {
    ...
    if (recordedCwd === null && unattributablePolicy === "summarize") {
      diagnostic?.({ code: "cwd-missing", runtime: runtime3 });
      unattributable?.({ reason: "cwd-missing", runtime: runtime3 });
      return null;
    }
    if (!meta && unattributablePolicy === "summarize") {
      diagnostic?.({ code: "metadata-prefix-incomplete", runtime: runtime3 });
      unattributable?.({ reason: "metadata-prefix-incomplete", runtime: runtime3 });
      return null;
    }
...
async function previewHandoffCandidates(sources, options = {}) {
  const deps = options.deps ?? DEFAULT_DEPENDENCIES3;
...
function canonicalize2(value2) {
  if (value2 === null || typeof value2 !== "object") return JSON.stringify(value2);
  if (Array.isArray(value2)) {
    return `[${value2.map(canonicalize2).join(",")}]`;
  }
  const record2 = value2;
  return `{${Object.keys(record2).toSorted().map((key) => `${JSON.stringify(key)}:${canonicalize2(record2[key])}`).join(",")}}`;
}
function sha256Canonical(value2) {
  return createHash2("sha256").update(canonicalize2(value2)).digest("hex");
}
...
`;
  return {
    provider: input.provider,
    status,
    receiptDigest: createHash3("sha256").update(receiptContents).digest("hex"),
    ...resolvedFailureStage === void 0 ? {} : { failureStage: resolvedFailureStage },
    reasonCodes
  };
}
async function verifyProviderBehavior(input) {
  const deps = input.deps ?? DEFAULT_DEPENDENCIES4;
...
async function git2(cwd, argv) {
  await execFileAsync3("git", ["-C", cwd, ...argv], {
    timeout: 1e4,
    maxBuffer: 65536,
    encoding: "utf8",
    shell: false,
...
async function createDefaultFixture() {
  ...
  await git2(sourceWorktree, ["init", "--initial-branch=main"]);
  await git2(sourceWorktree, ["config", "user.name", "Handoff Gate"]);
  await git2(sourceWorktree, ["config", "user.email", "handoff-gate@invalid"]);
  await git2(sourceWorktree, ["commit", "--allow-empty", "-m", "gate fixture"]);
  const targetWorktree = join3(repositoryRoot, "target-worktree");
  await git2(sourceWorktree, [
    "worktree",
    "add",
    "-b",
    "handoff-gate-target",
...
async function runDefaultProvider(invocationValue, executablePath) {
  try {
    const result = await execFileAsync3(executablePath, invocationValue.argv, {
...
async function snapshotTranscriptCandidate(provider2, candidate) {
  ...
  return {
    ...
    contentSha256: createHash3("sha256").update(contents).digest("hex")
  };
}
...
async function cleanupDefaultProvider(context, runCleanupCommand = async (executablePath, argv) => {
  await execFileAsync3(executablePath, argv, {
    timeout: 6e4,
    maxBuffer: 65536,
    encoding: "utf8",
    shell: false,
