...
- The current Session manifest explicitly labels live permission/discovery verification unverified. Follow [RELEASING.md](../../../RELEASING.md) per plugin; deterministic messaging evidence does not satisfy the separate live-host acceptance item.
...
Agent messaging is now a completed provider-neutral inbox implementation, independent of the stateless merged-log implementation. Session fidelity precedes the merged activity projection. Neither delivers safe N>2 transcript-consumer ownership automatically.
