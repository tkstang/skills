...
- The reviewer runs in the worktree with its own tools; the host writes a review request that names the scope and question and follows the session-handoff packet discipline. The rendered artifact records the scope, the commit hash, and the request verbatim, and echoes what the reviewer actually examined, so a review can be rerun and compared across providers.
