...
Round counts and wall-clock are partially tracked today; the v3 resolution block also calls for token counts, per-section round totals, and approximate cost where peer CLIs expose them. Add consistent metrics to turn records, loop status, and the artifact resolution block across iteration modes. Inventory what the owned `consensus` provider CLI and its adapters surface per run (cost/token reporting varies by provider) and degrade gracefully (omit rather than guess).
