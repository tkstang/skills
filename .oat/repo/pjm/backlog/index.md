...
- **September 16 review:** 19 open items; no whole item meets its acceptance criteria yet. See the [full review](reviews/backlog-and-roadmap-review.md), [roadmap](../roadmap.md), and [current-state snapshot](../current-state.md). The immediate kickoff order/capacity is awaiting operator confirmation; the older [priority alignment](reviews/priority-alignment.md) is historical until refreshed.
...
- **Immediate workflow:** **BL-260916-session-observer-re-armed — Investigate observer re-arm catch-up and suspected unread-record gaps** starts with reproduction, not an assumed fix. **BL-260916-add-consensus-review-cross — Add consensus-review: cross-model review of a bounded scope** is the user-selected high-priority feature. **BL-260916-add-a-first-party-install — Add a first-party install command for standalone skills** is an optional independent onboarding lane.
...
| BL-260916-session-observer-re-armed | Investigate observer re-arm catch-up and suspected unread-record gaps | open | medium | task | S |
