...
- **Independent collaboration projects:** **BL-260916-session-fidelity-opt — Session fidelity: opt-in --include-activity for observer and exporter** is implemented on its stacked delivery branches and archived; publication, merge, release, installation, and live provider acceptance remain separate. **BL-260619-inter-agent-direct-messaging — Inter-agent direct messaging (addressable, prioritized)** targets provider-neutral recipient inboxes, and **BL-260619-shared-session-log-substrate — Stateless multi-session activity merge** may reuse the completed activity contract. Messaging remains independent of that substrate. **BL-260713-per-observer-offsets-and-safe — Per-observer offsets and safe N>2 collaboration mesh** remains separate consumer-ownership work.
