...
**Last updated:** 2026-06-28 (**phone-a-friend** is complete: **BL-260620-add-phone-a-friend-advisory** shipped an instruction-only one-shot advisory peer skill, advisory schema, examples, manifests, tests, and Fumadocs/plugin documentation. Prior: consensus-family completed `independent_draft`, `create`, `decide`, and `plan`; docs IA shipped Fumadocs; provider-CLI hardening shipped submit-sidecar verdicts and conservative provider-exit retry classification; v0.1.0 released, with skills.sh hosted indexing still a non-claim until verified.)
