...
- **Session-observer collaboration v2 follow-ons** — the shipped skill remains N=2. Evidence-gated Cursor transcript-store/dotted-slug coverage and the finite stronger-wake investigation are complete; `buffered-manual` remains the strongest measured Cursor tier. Per-observer offset namespaces and safe N>2 mesh (**BL-260713-per-observer-offsets-and-safe**) plus optional idle-session application integrations (**BL-260713-optional-idle-session**) remain separate low-priority follow-ups. They must preserve the dependency-free core and honest validated-versus-unvalidated wake tiers; they do not supersede the shared-session-log or direct-messaging initiatives.
