# Kilo Code: native session storage reference
...
Legacy task directories live below an editor globalStorage root for `kilocode.kilo-code`. The current shared reader checks Code, Insiders, Cursor, Windsurf, remote-server locations, and provider-specific custom storage overrides. Each task can include `ui_messages.json`, `api_conversation_history.json`, `task_metadata.json`, and history/index companions.

Kilo has a separate SQLite path in addition to the legacy editor task format. Do not treat these as two copies of an identical JSON log.

The separate database path includes `~/.local/share/kilo/kilo.db` and KILO_DB/data-root overrides. The adapter inspects session/message/part/project tables. The minimal linked row schema only models evidenced identity/JSON carriers; inspect actual columns and decoded tool state before claiming complete support.
...
### kilo-code: legacy ui_messages.json entries

[JSON Schema](../schemas/native/kilo-code-ui.schema.json) · [Synthetic example](../examples/native/kilo-code-ui.json)
...
### kilo-code: legacy api_conversation_history.json

[JSON Schema](../schemas/native/kilo-code-api.schema.json) · [Synthetic example](../examples/native/kilo-code-api.json)
...
### Kilo Code: inspected SQLite row carrier subset

[JSON Schema](../schemas/native/kilo-code-sqlite-row.schema.json) · [Synthetic example](../examples/native/kilo-code-sqlite-row.json)

| Field | Type | Information carried |
|---|---|---|
| `id` | string | Row ID within the inspected table. |
| `session_id` | string | Owning session where applicable. |
| `message_id` | string | Owning message where applicable. |
| `data` | string | JSON payload carrier where present; complete tool-state schema not established here. |
