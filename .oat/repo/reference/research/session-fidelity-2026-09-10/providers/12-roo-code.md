# Roo Code: native session storage reference
...
Legacy task directories live below an editor globalStorage root for `rooveterinaryinc.roo-cline / roo-code.roo-cline`. The current shared reader checks Code, Insiders, Cursor, Windsurf, remote-server locations, and provider-specific custom storage overrides. Each task can include `ui_messages.json`, `api_conversation_history.json`, `task_metadata.json`, and history/index companions.

Roo discovery recognizes both extension identities; that is storage compatibility, not proof of identical behavior in every release.
...
### roo-code: legacy ui_messages.json entries

[JSON Schema](../schemas/native/roo-code-ui.schema.json) · [Synthetic example](../examples/native/roo-code-ui.json)
...
### roo-code: legacy api_conversation_history.json

[JSON Schema](../schemas/native/roo-code-api.schema.json) · [Synthetic example](../examples/native/roo-code-api.json)
