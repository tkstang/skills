...
- Post-tag discovery: `npx skills add tkstang/skills` resolves the repo's `SKILL.md` files from the tagged source, but the **public-discovery surface still needs control before any listing claim** (bl-7c1d). Only the standalone skills (`session-observer`, `export-session-transcript`) are self-contained and individually installable; the consensus `refine`/`evaluate` skills are **plugin-install-only** (they need `plugins/consensus/scripts/consensus.mjs`, which lives outside the skill dir, so an individual `npx skills add …@refine` fails `CONSENSUS_PROVIDER_CLI_MISSING`), and the `.agents/skills/oat-*` tooling must never be listed publicly. As of 2026-06-20, `tkstang/skills` is **not yet indexed** on skills.sh (`npx skills find session-observer|tkstang|oat-docs` returns nothing from this repo), so nothing is mis-exposed yet — no public skills.sh listing is claimed until the discovery surface is controlled and verified.
- Codex public Plugin Directory and skills.sh listing are explicitly not claimed until verified post-publication; recheck skills.sh indexing before any public-listing claim.
