...
Refine verified live with claude+codex across all three modes and the escalation ladder before the provider CLI cutover; current provider CLI parity is covered by focused integration tests, smoke, `premerge`, and provider inventory/preflight evidence. Evaluate has mocked end-to-end coverage and operator QA docs in `skills/evaluate/references/operator-qa.md`; live provider verification remains part of v0.1 release checks. Cursor-as-peer remains blocked by local auth/keychain state until an authenticated provider CLI run verifies it end-to-end.

Not yet implemented (see `roadmap.md`): the other four family skills (`consensus-create|decide|plan|research`), whole-document harmonization, deliberation metrics/cost caps, the deferred independent-draft cold-start strategy, a convergence similarity heuristic (deterministic-only triggers shipped; bl-ef38), tool-based verdict submission hardening (bl-3a88), and Cursor authenticated end-to-end verification (bl-f0b6).
