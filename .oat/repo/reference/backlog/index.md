...
- **Provider CLI / peer invocation (seeded 2026-06-13; shipped 2026-06-19):** bl-bb7e is now **done**. Refine/Evaluate new runs use the owned `consensus` provider CLI for provider inventory, preflight, bounded subprocess execution, provider-neutral diagnostics, schema delivery, and retry/cap/timeout ownership. Remaining follow-ups are narrower: bl-3a88 for a future submit-tool/self-validation path and bl-f0b6 for authenticated Cursor end-to-end verification through the provider CLI.
