...
- **Release:** bl-d85f (v0.1 verification + tag) is in progress on the release-verification branch. Current automated gates, release workflow parity, version/tag checks, local Claude/Codex install evidence, and Cursor authenticated peer E2E are refreshed; PR #9 remains reused evidence for live `consensus-refine` mode/escalation dogfood. Remaining gates are interactive provider permission prompts, provider plugin-install checks still called out in `RELEASING.md`, and post-tag skills.sh/public discovery verification before public claims.
