...
- **v3 family deferrals (seeded 2026-06-19):** **bl-645c** remains the final family wrapper and is design-gated on peer tool access / evidence capture; it uses `shared_input` rather than `independent_draft`. **bl-f8cb** is a low-priority seed for the parked v3 "3+ peer" extension so the two-peer constraint is explicit.
