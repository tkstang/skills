...
**Context:** The canonical refine wrapper TypeScript source needs to type-check against `src/consensus/core/consensus-loop.ts`, but the shipped wrapper runtime must continue importing the sibling provider-facing `./consensus-loop.mjs` file from `plugins/consensus/skills/refine/scripts/`.
**Decision:** Extend DR-020's generated-output build with optional per-mapping `importRewrites`. `src/consensus/refine/consensus-refine.ts` imports the loop through the NodeNext-resolvable canonical specifier `'../core/consensus-loop.js'`; `scripts/build-generated.mjs` rewrites that exact emitted specifier to `'./consensus-loop.mjs'` when producing `plugins/consensus/skills/refine/scripts/consensus-refine.mjs`. A declared rewrite fails loudly if the `from` specifier is absent.
**Rationale:** This keeps TypeScript checking wired to the real canonical loop API while preserving the existing dependency-free shipped runtime layout. The transform is intentionally narrow, explicit in build config, compatible with esbuild `bundle:false`, and covered by drift and generated-import tests so a path regression cannot silently ship.
**Status:** Accepted.
