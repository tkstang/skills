...
**Last updated:** 2026-06-21 (**docs IA** (`bl-ecaa`) is **done** (2026-06-21): a Fumadocs docs site was stood up at `documentation/` with a two-trunk audience-split IA (User Guide + Engineering), the dense README was migrated into the site and slimmed to a lean entry point, and a GitHub Pages deploy workflow was added (`.github/workflows/deploy-docs.yml`); framework decision recorded as DR-024. Prior: planning pass committed a 3-track execution plan — **consensus-family** (`bl-2ed7` gate → `bl-b9b9` → `bl-87ef`/`bl-0cb8`, one project) running in parallel with **provider-cli-hardening** (`bl-3a88` design-first + `bl-3291`, disjoint source surface), with **docs IA** (`bl-ecaa`) promoted to immediate post-tag. `bl-d85f` (v0.1 verification + tag) is **done** (2026-06-20): all gates green, v0.1.0 tagged from `main`, only post-tag skills.sh discovery remains as a non-claim. See `backlog/reviews/priority-alignment.md`. Prior: Cursor authenticated peer E2E through the provider CLI is verified for Refine and Evaluate with `cursor,codex`, completing **bl-f0b6**. Prior: consensus peer invocation now defaults to the owned provider CLI for Refine/Evaluate, completing **bl-bb7e** and superseding the old external peer-run boundary (DR-023). consensus-rubric-guidance brought `refine` + `evaluate` to authoring-best-practice parity with a validator-backed top-level skill `version` (DR-022), plus guided rubric creation and four bundled example rubrics. Also 2026-06-19: v0.1 release verification refreshed automated gates, provider install evidence, release notes, version/tag checks, and release workflow parity; test-organization cleanup landed shared `tests/helpers/`, a domain-organized test tree, and two oversized suites split. Prior: PR4 retired the `node:test` runner and completed **bl-bfb4**.)
...
- **Provider-CLI hardening (active track)** — one OAT project on the `provider-cli/` surface: **bl-3a88** (tool-based verdict submission) **design pass first** so the decision lands before the synthesized family wrappers fan out, then build; **bl-3291** (provider-exit transient-vs-terminal retry classification) as an additive ride-along.

## Next

- **Docs IA (done 2026-06-21)** — **bl-ecaa**: stood up a Fumadocs docs site at `documentation/` with a two-trunk audience-split IA (User Guide + Engineering), migrated the dense README into the site and slimmed it (205 → 72 lines) to an entry point, and added a GitHub Pages deploy workflow (`.github/workflows/deploy-docs.yml`). Framework decision: Fumadocs (DR-024). Operator must enable Settings → Pages → Source: GitHub Actions for the first deploy. With the site live, project documentation now targets it via `oat-project-document`, not the README — so the family project can document itself into the site.
...
- **Advisory peer (phone-a-friend)** — **bl-22d3**: a one-shot structured second opinion over the provider CLI (no deliberation loop). Buildable any time; the "sequenced after docs IA" gate is now **satisfied** — docs IA has landed, so bl-22d3 documents into the live site and is the first validation that the IA absorbs a new skill cleanly.
