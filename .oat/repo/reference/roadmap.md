...
- v0.1 tag gated on `RELEASING.md` manual provider verification: automated gates and local Claude/Codex installs are current, Cursor authenticated peer E2E is verified, and remaining gates are interactive permission prompts plus post-tag Agent Skills / skills.sh discovery.
...
- **v0.1 release verification (in flight)** — bl-d85f is in flight in a release worktree as a **finishing pass**. Already green (reuse, don't redo): automated gates, version/tag readiness, release workflow parity, local Claude/Codex installs, and Cursor authenticated peer E2E. Remaining gates only: interactive provider permission prompts, CHANGELOG/version/tag finalization, README install-matrix re-confirmation, tag push + green `release.yml`, and post-tag discovery. Authoritative status: the `RELEASING.md` v0.1 Readiness Snapshot.
