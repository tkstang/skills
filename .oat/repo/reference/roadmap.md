...
**Last updated:** 2026-06-22 (**consensus-family** is implementation-complete: **bl-2ed7**, **bl-b9b9**, **bl-87ef**, and **bl-0cb8** are done; `independent_draft` is now a shared loop cold-start, and `consensus-create`, `consensus-decide`, and `consensus-plan` ship as generated TypeScript-backed wrappers documented in the Fumadocs site. **docs IA** (`bl-ecaa`) is done: a Fumadocs docs site was stood up at `documentation/`; framework decision recorded as DR-025. **Provider-CLI hardening** is complete: **bl-3a88** shipped the submit-CLI verdict path and **bl-3291** locked/audited conservative provider-exit retry classification (DR-024). `bl-d85f` (v0.1 verification + tag) is done: **v0.1.0 released** — tag on `main`, `release.yml` green, GitHub Release published; skills.sh hosted indexing remains a non-claim until verified.)
...
- **Consensus family (done 2026-06-22)** — **bl-2ed7** (`independent_draft` cold-start), **bl-b9b9** (`consensus-create`), **bl-87ef** (`consensus-decide`), and **bl-0cb8** (`consensus-plan`) are complete.
...
- **Consensus research** — **bl-645c**: the last family skill, kept as a separate project because peer tool access, evidence capture, and permissions need an explicit design/DR before build. It uses `shared_input`, so it is not gated on the completed `independent_draft` work.
...
- Harmonization pass (bl-e39a) and deliberation metrics (bl-9ed4) — after create/decide/plan land, per v3 Phase 4. Similarity heuristic (bl-ef38) alongside.
