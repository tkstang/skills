...
- **v0.1 release verification** — bl-d85f is in progress on the release-verification branch. Completed so far: current automated gates, README/CHANGELOG/version/tag checks, release workflow parity, local Claude/Codex install evidence, and Cursor authenticated peer E2E. Remaining before tag: interactive provider permission prompts and any provider plugin-install checks still called out in `RELEASING.md`.
...
- **Peer-invocation hardening** — tool-based verdict submission (bl-3a88) remains a later reliability design now that the provider CLI owns the claude/codex/cursor path and Cursor-as-peer E2E is verified.

## Later

- Harmonization pass and deliberation metrics (after the family ships, per v3 Phase 4).
- Cursor submit-tool / custom ACP provider path exploration.
