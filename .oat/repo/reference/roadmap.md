...
**Last updated:** 2026-06-21 (**docs IA** (`bl-ecaa`) is **done**: a Fumadocs docs site was stood up at `documentation/` with a two-trunk audience-split IA (User Guide + Engineering), the dense README was migrated into the site and slimmed to a lean entry point, and a GitHub Pages deploy workflow was added (`.github/workflows/deploy-docs.yml`); framework decision recorded as DR-025. **Provider-CLI hardening** is also complete: **bl-3a88** shipped the submit-CLI verdict path with sidecar capture, Codex strict-output avoidance, bounded submit capture, deterministic evidence, and gated live Codex workspace-write E2E; **bl-3291** locked/audited conservative provider-exit retry classification (DR-024); the consensus-family track can now plan against the decided submit-CLI + parse-fallback contract. `bl-d85f` (v0.1 verification + tag) is done: **v0.1.0 released** — tag on `main`, `release.yml` green, GitHub Release published; only skills.sh hosted indexing remains a non-claim. Prior: Cursor authenticated peer E2E completing **bl-f0b6**; consensus peer invocation defaults to the owned provider CLI (DR-023); consensus-rubric-guidance brought `refine` + `evaluate` to authoring-best-practice parity (DR-022) with guided rubric creation and four bundled example rubrics; PR4 retired the `node:test` runner (**bl-bfb4**). See `backlog/reviews/priority-alignment.md`.)
