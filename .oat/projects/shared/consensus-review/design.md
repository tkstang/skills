...
- **Rendering:** one fixture per severity plus mixed/empty findings, document anchors, exact request preservation, Markdown-injection payloads, stable IDs, escaping, and separation of checks run from proposed checks. Run the independent OAT receipt exercise before claiming that acceptance criterion complete.
