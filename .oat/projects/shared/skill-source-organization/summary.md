...
This project reorganized the repository so each shipped skill has one authored source directory while standalone and plugin installations are generated outputs. The public milestone implements issue #74, introduces separate session and consensus plugin boundaries, and preserves dependency-free Node.js runtime packages without claiming unpublished marketplace or live-provider readiness.

P01 through p04 are complete and independently reviewed. The project remains open for p05, which follows the public merge and removes the transitional independently authored `complexity-review` copy from the private personal-skills repository under separate authorization.
...
- The p04 public branch must merge before p05 changes the private personal-skills source or any user-level install.

## Follow-up Items

- Publish and merge the reviewed p01-p04 progress PR after explicit authorization.
- After the public replacement is merged and installable, execute p05 in the private personal-skills repository: remove its independently authored `complexity-review` copy, update declarations and docs, regenerate outputs, and open a separate linked PR. Do not merge that PR or modify active installations automatically.
