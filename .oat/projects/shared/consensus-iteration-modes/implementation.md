...
| p06-t06 | plan.md Phase 6 (NFR4) | Mode-comparison dogfood: run refine in all 3 modes on a real doc, review artifacts | PARTIALLY run 2026-06-13 (laptop worktree); blocked on provider auth | Requires `paseo` + real peer CLIs + live spend. Dogfood validated the wrapper pipeline end-to-end and surfaced two real issues (see fix row below + QA-guide prereq hardening), but could not complete a converging run because only `codex` was `available` on the laptop daemon (claude/copilot/opencode/pi `unavailable` — auth not loaded). | manual follow-up | User authenticates a 2nd paseo provider (claude), then re-runs the operator-qa.md scenarios. NFR4 close-out pending that run. |
| dogfood fix | schemas (draft) | Output schemas declared JSON Schema draft 2020-12 (inherited from v0.1) | Switched all 3 schemas to draft-07 (commit ea45752) | Live `paseo run` failed `AGENT_CREATE_FAILED: no schema with key/ref draft/2020-12` — paseo builds output-schema validators with a default ajv (draft-07) that cannot resolve the 2020-12 meta-schema. Stubbed tests never hit paseo's ajv (hand-rolled JS validators ignore `$schema`), so this pre-existing bug (v0.1 had it too — the v0.1 plugin was never run end-to-end against live paseo) was invisible until dogfooding. Schemas use only draft-07-compatible keywords, so behavior is unchanged. | implementation (fixed) | None — fixed. Validates the dogfood's worth: it caught a bug that blocked ALL live runs and that 513 stubbed tests could not. |
