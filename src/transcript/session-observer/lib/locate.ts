...
const EXACT_ALL_DISCOVERY_BUDGET: DiscoveryBudgetOptions = {
  maxEntries: 50_000,
...
export type SessionDiscoveryFailure =
...
export class SessionDiscoveryError extends Error {
  readonly code: SessionDiscoveryFailure;

  constructor(code: SessionDiscoveryFailure) {
    super(code);
...
class ExactAllDiscoveryBudget {
  ...
  constructor(
    readonly limits: DiscoveryBudgetOptions,
    private readonly runtime: Runtime,
    private readonly diagnostic?: DiscoveryOptions['diagnostic'],
  ) {}

  private emit(code: 'deadline-exceeded' | 'budget-exceeded'): void {
...
function exactAllBudget(
  runtime: Runtime,
  options: DiscoveryOptions | undefined,
): ExactAllDiscoveryBudget | null {
...
class CursorDiscoveryBudget {
  ...
  constructor(options: CursorDiscoveryBudgetOptions = {}) {
    ...
    this.maxRetainedCandidates =
      options.maxRetainedCandidates ?? CURSOR_DISCOVERY_MAX_RETAINED_CANDIDATES;
...
// (and its sessionId/recordedCwd meta) is deterministic given its content,
// and (mtimeMs, size) is this codebase's established change signature (see
// watch.ts's FileSignature), so a signature-keyed cache is a safe,
// transparent memoization: a hit returns byte-identical results to a fresh
// read, and any content change (which always changes size, and in practice
// mtime too) invalidates it.
//
// Each cache entry holds BOTH the classification and the meta
// (sessionId/recordedCwd) derived from the same transcript, because
// candidate discovery needs both and they come from the same parsed record
// array — caching only one and still separately re-reading for the other
// would leave the full-file read+parse on the hot path regardless. A miss
// reads and parses once and derives both; a hit costs no I/O for either.
// (Codex is the one runtime that does not read its meta through this path —
// it already has its own persistent, cross-process cwd cache, see below —
// so for codex the meta half of a cache entry is simply unused, not wrong.)
//
...
export class ClassificationCache {
  ...
  constructor(maxEntries: number = DEFAULT_CLASSIFICATION_CACHE_MAX_ENTRIES) {
...
const UNKNOWN_CLASSIFICATION: TranscriptClassification = {
  status: 'unknown',
  engaged: true,
  recordCount: null,
...
/**
 * Read, classify, and extract meta for one transcript in a single pass,
 * consulting and populating `cache` by (path, mtimeMs, size). A cache hit
...
async function candidateDerivedFields(
  ...
  try {
    ...
    // Not cached: a transient read failure should not poison future calls
    // once the file becomes readable again.
    return { meta: null, classification: UNKNOWN_CLASSIFICATION };
  }
}

async function candidateDerivedFieldsBounded(
  runtime: Runtime,
  transcriptPath: string,
  signature: { mtimeMs: number; size: number },
  cache: ClassificationCache,
  ...
  let transcriptIssue: DiscoveryUnattributableReason | null = null;
  const read = await readMetadataRecordsBounded(transcriptPath, {
    ...
    diagnostic: ({ code }) => {
      ...
      if (
        ...
        ['malformed-record', 'oversized-record', 'read-failed'].includes(code)
      ) {
        transcriptIssue = code as DiscoveryUnattributableReason;
      }
...
async function collectJsonlFiles(
  dir: string,
  budget: ExactAllDiscoveryBudget | null = null,
): Promise<string[]> {
...
async function discoverCodex(
  ...
  for (const transcriptPath of allFiles) {
    ...
    candidates.push({
      ...
      ageSec,
      ...(boundedDerived
        ? engagementCandidateFields(boundedDerived.classification)
...
async function findCursorSessionCandidates(
  ...
  const pinnedBudget = new CursorDiscoveryBudget({
    maxEntries:
      cursorDiscoveryTestOptions?.maxEntries ??
      CURSOR_IDENTITY_INDEX_MAX_ENTRIES,
    maxElapsedMs:
      cursorDiscoveryTestOptions?.maxElapsedMs ??
      CURSOR_IDENTITY_INDEX_MAX_ELAPSED_MS,
...
export async function resolveCursorIdentity(
  ...
  const canonicalCwd =
    requestedCanonicalCwd ?? requestedCwd.replace(/\/+$/u, '');
  const canonicalTranscriptPath =
    resolvedTranscriptPath ?? candidate.transcriptPath;
  ...
  if (
    resolvedStoreRoot !== null &&
    resolvedTranscriptPath !== null &&
    !pathIsWithin(canonicalStoreRoot, canonicalTranscriptPath)
  ) {
  ...
  const canonicalIdentityReady =
    requestedCanonicalCwd !== null &&
    resolvedTranscriptPath !== null &&
    resolvedStoreRoot !== null &&
    (candidate.recordedCwd === null || canonicalRecordedCwd !== null);
