...
export function canonicalJsonBlock(label: string, value: unknown) {
...
export function latestRevisedOutput(
...
export function fallbackErrorStatus(
  error: unknown,
  records: ConsensusRecord[],
  peerCount: number,
): SectionStatus {
...
export function aggregateStatus(sections: SectionResult[]) {
...
export function aggregateParallelStatus(sections: SectionResult[]) {
  const statuses = sections.map(
...
export function sectionStates(sections: SectionResult[]) {
