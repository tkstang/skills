...
const OWNER_RUNTIMES = new Set(['claude-code', 'codex', 'cursor']);
...
export function validateLease(raw) {
  ...
  if (value.runtime === 'claude-code') {
    ...
    if (
      ...
      composition.controller !== 'observer-collab' ||
      composition.mechanism !== 'monitor' ||
      composition.ownerRuntime !== value.runtime ||
      composition.ownerSession !== value.ownerSession ||
      composition.peerRuntime !== value.peerRuntime ||
      composition.peerSession !== value.peerSession ||
      composition.ownerCwd !== value.ownerCwd ||
      composition.peerTranscript !== value.peerTranscript ||
      typeof composition.confirmedAt !== 'string' ||
      !Number.isFinite(Date.parse(composition.confirmedAt)) ||
      composition.oldMonitorStopped !== true ||
      composition.standaloneWatcherStopped !== true
    ) {
      throw new LeaseError(
    ...
    value.composedActivation !== undefined &&
    value.composedActivation !== null
  ) {
    throw new LeaseError(
      'invalid-composed-activation',
