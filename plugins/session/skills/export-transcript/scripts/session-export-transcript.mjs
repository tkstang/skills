...
function unavailableActivityReport(source, sourceBytes, capturedAt, totalRecords) {
...
async function exportSession(opts, runtime, branch, branchFromGit, session, multi) {
  const capturedRead = opts.includeActivity ? await readRecordsDetailed(session.transcriptPath) : void 0;
  const records = capturedRead ? capturedRead.records.map(({ record }) => record) : await readRecords(session.transcriptPath);
