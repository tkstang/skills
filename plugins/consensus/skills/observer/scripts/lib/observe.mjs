...
function cwdCachePath() {
  const stateDir3 = process.env.STATE_DIR ?? join3(homedir3(), ".local", "state", "session-observer");
  return join3(stateDir3, "codex-cwd-cache.json");
}
    ...
    if (firstObservation !== null) {
      ...
      if (checkpoint.status === "confirmed") {
        ...
        if (remainingMs === null || stabilityMs <= remainingMs) {
          ...
          await (deps.sleep?.(waitMs) ?? new Promise((resolve2) => setTimeout(resolve2, waitMs)));
