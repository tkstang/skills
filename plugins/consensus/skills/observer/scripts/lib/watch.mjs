...
function topLevelResultOutcome(toolUseResult) {
  if (!isJsonObject2(toolUseResult)) return "unknown";
...
function externalReference(toolUseResult) {
  if (!isJsonObject2(toolUseResult)) return void 0;
...
function topLevelToolUseResultActivity(source, detailed, origin) {
  ...
  const status = isJsonObject2(toolUseResult) ? toolUseResult.interrupted === true ? "interrupted" : stringValue(toolUseResult.status) : void 0;
  return {
    events: [
      {
...
function claudeSystemActivity(source, detailed) {
  ...
  if (subtype === "compact_boundary") {
    const locator = recordLocator(detailed, "");
    const compactMetadata = isJsonObject2(record.compactMetadata) ? record.compactMetadata : void 0;
...
function extractClaudeRecord(source, detailed) {
  ...
  const sourceSkillNamesRecorded = record.type === "attachment" && isJsonObject2(record.attachment) && (record.attachment.type === "skill_listing" && Array.isArray(record.attachment.names) || record.attachment.type === "invoked_skills" && Array.isArray(record.attachment.skills));
  ...
  if (Array.isArray(content)) {
    content.forEach((candidate, blockIndex) => {
      if (!isJsonObject2(candidate)) return;
...
function codexCallArguments(nativeType, payload, locator) {
  ...
  try {
    ...
    if (!isJsonObject2(parsed)) throw new Error("arguments are not an object");
    return {
...
function selectedSessionMetadata(payload) {
  ...
  const source = isJsonObject2(payload.source) ? payload.source : void 0;
  const subagent = source && isJsonObject2(source.subagent) ? source.subagent : void 0;
  const threadSpawn = subagent && isJsonObject2(subagent.thread_spawn) ? subagent.thread_spawn : void 0;
...
function itemCompletedActivity(source, detailed, payload) {
  const item = isJsonObject2(payload.item) ? payload.item : void 0;
...
function extractCodexRecord(source, detailed) {
  const { record } = detailed;
  const payload = isJsonObject2(record.payload) ? record.payload : void 0;
...
function tokenFields(value) {
  if (!isJsonObject2(value)) return void 0;
  const entries = [];
...
function claudeUsage(source, records) {
  ...
  for (const detailed of records) {
    const { record } = detailed;
    if (record.type !== "assistant" || !isJsonObject2(record.message)) continue;
...
function codexUsage(source, records, events) {
  ...
  for (const detailed of records) {
    const { record } = detailed;
    if (record.type !== "turn_context" || !isJsonObject2(record.payload))
      continue;
  ...
  for (const detailed of records) {
    ...
    if (record.type === "event_msg" && payload?.type === "token_count") {
      const info = isJsonObject2(payload.info) ? payload.info : void 0;
      if (!info) continue;
