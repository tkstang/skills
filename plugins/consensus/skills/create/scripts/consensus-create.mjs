...
async function confineRead(inputPath, cwd, rootPath) {
  ...
  const realTarget = await realpath2(target);
...
async function loadCreateInputs(options, { cwd = process.cwd() } = {}) {
  const resolvedCwd = path7.resolve(cwd);
  const allowedRoot = path7.resolve(options.allowRoot ?? resolvedCwd);
  const briefPath = options.briefFile ? await confineRead(options.briefFile, resolvedCwd, allowedRoot) : null;
...
function untrustedCreateInputBlocks(inputs) {
  const blocks = [
...
function createPeerPrompt(input) {
...
function buildCreatePromptProfile(inputs) {
  return {
...
async function resolveOutputPath(options) {
  const cwd = path7.resolve(options.cwd ?? process.cwd());
  const root = path7.resolve(options.allowRoot ?? cwd);
  const target = options.output ? path7.isAbsolute(options.output) ? options.output : path7.resolve(cwd, options.output) : path7.resolve(cwd, "consensus-create.md");
  return await confineWrite(target, root);
}
...
async function runConsensusCreate(input, runOptions = {}) {
  ...
  const inventory = normalized.peers === null ? await loadCreateProviderInventory({ env, cwd }) : void 0;
...
async function runCreateCli(argv = process.argv.slice(2), options = {}) {
