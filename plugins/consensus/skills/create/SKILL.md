...
Resolve the exact peers and synthesizer this run will call from explicit flags and
...
or `unsupported`, stop before provider calls and relay the named provider, status,
and remediation hint. Do not fetch installers, probe another provider's auth, or
retry blindly.
