...
Do not use it as a replacement for a one-time `observer review`, and do
...
1. In each session, run the base one-liner `observer whoami --json`.
...
3. Start one pinned base `catch-up-then-watch` per stateful observer, normally
   ...
   `observer catch-up-then-watch --session <runtime>:<id> --quiet-empty`
...
`observer` workflow and every observer public/private offset
...
Use only: `observer review --session <runtime:id>` for either peer.
