...
function parsePanelSize2(value) {
  if (!/^\d+$/u.test(value)) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (token) {
      ...
      case "--panelists":
        ...
        break;
      case "--panel-size":
        parsed.panelSize = parsePanelSize2(requireValue(argv, index, token));
        index += 1;
        break;
      case "--output":
...
async function atomicWritePanelFile(targetPath, contents, options = {}) {
  ...
  try {
    ...
    await rename2(tempPath, writePath);
  } catch (error) {
    try {
...
async function readPanelQuestionFile(inputPath) {
  ...
  const contents = await readFile2(inputPath, "utf8");
...
function assertKnownKeys2(record, knownKeys, label) {
  for (const key of Object.keys(record)) {
    if (!knownKeys.has(key)) {
      throw new Error(`${label} has unknown key: ${key}`);
    }
...
async function runConsensusPanel(input, runOptions = {}) {
  ...
  await mkdir2(paths.runDir, { recursive: true });
