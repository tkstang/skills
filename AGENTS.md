...
- Keep `CHANGELOG.md` current in the same change: any PR that bumps a canonical skill `metadata.version`, changes a plugin release version, or changes user-facing behavior adds an entry under `## [Unreleased]` (Added / Changed / Fixed / Removed) naming the affected skills and their new versions. At release, the maintainer moves the Unreleased entries under the new version heading with its date. A version bump without a changelog entry is a review finding.
