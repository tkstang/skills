...
- Generated runtime outputs may also come from canonical TypeScript source under `src/`. Edit the canonical TypeScript source, run `pnpm run build` to regenerate the committed `.mjs` runtime output under `plugins/`, and use `pnpm run build:check` or `tests/generated-output-sync.test.mjs` to catch drift. Never hand-edit generated `.mjs` outputs with a `// GENERATED` banner, including `plugins/consensus/skills/refine/scripts/consensus-loop.mjs` and `plugins/consensus/skills/refine/scripts/consensus-refine.mjs`.
