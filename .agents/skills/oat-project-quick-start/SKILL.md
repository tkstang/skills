...
When `OAT_AUTONOMOUS=1` and the run reaches the step 5 prompt, use that existing
proceed-anyway branch, record gate `QS-01`, and leave all unrelated working-tree
files unstaged. A cleanup that could discard work remains a destructive-change
boundary. A run that took the step 2 manifest-only path never reaches this
branch and records no gate.
