...
```text
pnpm exec vitest run src/skills/session-observer-collab/src/cursor-hook.test.ts src/skills/session-observer-collab/src/control.test.ts src/skills/session-observer-collab/src/completion.test.ts src/skills/session-observer-collab/src/wake-envelope-contract.test.ts
pnpm exec vitest run src/shared/transcript/runtimes.test.ts
```
...
| Streaming frame and terminal variants | Synthetic repository fixtures    | Cursor transcript fixture contract | Repository revision        | `src/skills/session-observer/src/fixtures/cursor/framed-*.jsonl`      | Repository-relative fixture paths only                                                   | Closed, blank, malformed-middle, partial-tail, repaired, appended, and same-length-replaced frames                          | No session, lease, or provider identity stored                                                                      | Focused fixture and reader suites                                | Structural scenarios pass automated checks; fixture prose is explicitly synthetic                                     | `automated-only` |
