...
function completedTurns(entries, fromIndex) {
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    const classification = text.trim().length === 0 ? "empty-turn" : NO_OP_PREFIX.test(text) ? "no-op-turn" : automaticWake && (ACKNOWLEDGMENT.test(text) || STATUS_ECHO.test(text)) ? "automatic-control-turn" : "substantive-turn";
    turns.push({
      fromIndex: start,
