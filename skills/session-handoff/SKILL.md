...
The `session-observer` and `session-export-transcript`
...
`session-observer` is optional and its installed contract decides which
...
When the installed `session-export-transcript` skill is available, offer its
...
After acceptance, invoke `session-export-transcript` under its installed
