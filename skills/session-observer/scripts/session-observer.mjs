...
function topLevelResultOutcome(toolUseResult) {
  if (!isJsonObject3(toolUseResult)) return "unknown";
...
function externalReference(toolUseResult) {
  if (!isJsonObject3(toolUseResult)) return void 0;
  const path = stringValue2(toolUseResult.persistedOutputPath);
...
function childReference(toolUseResult) {
  if (!isJsonObject3(toolUseResult)) return void 0;
  const nativeId = stringValue2(toolUseResult.agentId);
  if (!nativeId) return void 0;
  const nickname = stringValue2(toolUseResult.description);
  const status = stringValue2(toolUseResult.status);
  return {
    nativeId,
...
function topLevelToolUseResultActivity(source, detailed, origin) {
  ...
  const status = isJsonObject3(toolUseResult) ? toolUseResult.interrupted === true ? "interrupted" : stringValue2(toolUseResult.status) : void 0;
  return {
    events: [
      {
...
function selectedClaudeMetadata(record) {
  ...
  const model = message ? stringValue2(message.model) : void 0;
  const effort = stringValue2(record.effort);
  const perTurnEffort = stringValue2(record.perTurnEffort);
  const timestamp = stringValue2(record.timestamp);
...
function claudeSystemActivity(source, detailed) {
  ...
  const subtype = stringValue2(record.subtype);
  ...
  if (subtype === "compact_boundary") {
    const locator = recordLocator(detailed, "");
    const compactMetadata = isJsonObject3(record.compactMetadata) ? record.compactMetadata : void 0;
    const trigger = compactMetadata ? stringValue2(compactMetadata.trigger) : void 0;
...
function extractClaudeRecord(source, detailed) {
  ...
  if (Array.isArray(content)) {
    content.forEach((candidate, blockIndex) => {
      if (!isJsonObject3(candidate)) return;
      const blockType = stringValue2(candidate.type);
      ...
      if (blockType === "tool_result") {
        const nativeCallId = stringValue2(candidate.tool_use_id);
...
function codexCallArguments(nativeType, payload, locator) {
  ...
  try {
    ...
    if (!isJsonObject3(parsed)) throw new Error("arguments are not an object");
    return {
...
function selectedSessionMetadata(payload) {
  const cliVersion = stringValue2(payload.cli_version);
  const modelProvider = stringValue2(payload.model_provider);
  const nativeSessionId = stringValue2(payload.id);
  const directParentMarkerPresent = Object.hasOwn(payload, "parent_thread_id");
  const directParentThreadId = stringValue2(payload.parent_thread_id);
  const source = isJsonObject3(payload.source) ? payload.source : void 0;
  const subagent = source && isJsonObject3(source.subagent) ? source.subagent : void 0;
  const threadSpawn = subagent && isJsonObject3(subagent.thread_spawn) ? subagent.thread_spawn : void 0;
  const nestedParentThreadId = threadSpawn ? stringValue2(threadSpawn.parent_thread_id) : void 0;
...
function selectedTurnMetadata(payload) {
  const model = stringValue2(payload.model);
  const effort = stringValue2(payload.effort);
  return {
    ...model === void 0 ? {} : { model },
    ...effort === void 0 ? {} : { effort }
  };
...
function responseItemActivity(source, detailed, payload) {
  ...
  if (nativeType === "function_call" || nativeType === "custom_tool_call") {
    const nativeCallId = stringValue2(payload.call_id);
    const nativeId = stringValue2(payload.id);
...
function itemCompletedActivity(source, detailed, payload) {
  const item = isJsonObject3(payload.item) ? payload.item : void 0;
  ...
  const nativeType = stringValue2(item.type);
  ...
  const nativeId = stringValue2(item.id);
  const nativeCallId = stringValue2(item.call_id);
  const nativeStatus = stringValue2(item.status);
  const turnId = stringValue2(payload.turn_id);
  const childNativeId = stringValue2(item.agent_thread_id);
  const childNickname = stringValue2(item.agent_nickname);
...
function cwdCachePath() {
  const stateDir5 = process.env.STATE_DIR ?? join2(homedir2(), ".local", "state", "session-observer");
  return join2(stateDir5, "codex-cwd-cache.json");
}
...
function isCursorState(value) {
  if (!isObject2(value) || !hasOnlyKeys(value, ["schemaVersion", "sessions", "legacyUnverified"]) || value.schemaVersion !== SCHEMA_VERSION2 || !isObject2(value.sessions) || !isObject2(value.legacyUnverified)) {
    return false;
  }
...
function statePath(dir) {
  return join3(dir, "cursor-state.json");
}
function lockPath(dir) {
  return join3(dir, "cursor-state.json.lock");
}
function tempPath(dir) {
  return join3(dir, `cursor-state.json.${process.pid}.tmp`);
}
...
async function createLockContender(path, owner, options = {}) {
  ...
  let handle;
  try {
    handle = await open4(privatePath, "wx", 384);
...
async function contenderOwnsTurn(path, ownerPath, owner) {
  ...
  for (const name of (await readdir(owners)).toSorted()) {
    ...
    const contenderPath = join3(owners, name);
    let contenderOwner;
    try {
      contenderOwner = await readFile3(contenderPath, "utf8");
    } catch (error) {
    ...
    if (!processIsAlive(pid)) {
      await unlink2(contenderPath).catch((error) => {
        if (!isErrnoException(error) || error.code !== "ENOENT") throw error;
      });
      continue;
    }
...
async function releaseLock(path, ownership) {
  try {
    if (await readFile3(path, "utf8") === ownership.owner) {
      await unlink2(path);
    }
  } catch (error) {
    if (!isErrnoException(error) || error.code !== "ENOENT") throw error;
...
async function writePrivateAtomic(path, temporaryPath, content) {
  let handle;
  try {
    handle = await open4(temporaryPath, "w", 384);
...
async function readCursorState(dir) {
  let raw;
  try {
    raw = await readFile3(statePath(dir), "utf8");
  } catch (error) {
    ...
    if (firstObservation !== null) {
      ...
      if (checkpoint.status === "confirmed") {
        ...
        if (remainingMs === null || stabilityMs <= remainingMs) {
          ...
          await (deps.sleep?.(waitMs) ?? new Promise((resolve4) => setTimeout(resolve4, waitMs)));
...
var CONTROL_DIRECTIVES = /* @__PURE__ */ new Set(["flush", "pause", "resume", "stop"]);
...
function activityAccountingSignal(digest) {
...
var VALID_RUNTIME_LABEL2 = VALID_RUNTIMES2.join(", ");
var VALID_WATCH_RUNTIMES = [...VALID_RUNTIMES2, "auto", "both"];
var VALID_WATCH_RUNTIME_LABEL = VALID_WATCH_RUNTIMES.join("|");
function isRuntime2(value) {
  return typeof value === "string" && VALID_RUNTIMES2.includes(value);
}
async function preferredRuntimeFromState2(withCandidates, targetCwd) {
  let state;
  try {
    state = await load();
  } catch {
...
async function resolveAutoRuntime2(targetCwd) {
  const self = process.env.SESSION_OBSERVER_SELF;
  const results = await Promise.all(
    VALID_RUNTIMES2.map(async (rt) => {
      try {
        const candidates = await discover(rt, targetCwd);
        return { runtime: rt, candidates };
      } catch {
  ...
  const withCandidates = results.filter((r) => r.candidates.length > 0);
  const considered = isRuntime2(self) ? withCandidates.filter((r) => r.runtime !== self) : withCandidates;
  ...
  const preferred = await preferredRuntimeFromState2(considered, targetCwd);
  if (preferred) return preferred;
  return {
    ambiguous: true,
    runtimes: considered.map((r) => r.runtime),
...
var StdoutWriteError = class extends Error {
  asynchronous;
...
function unengagedOnlyMessage2(runtime, cwd) {
  return `The only ${runtime} session for this cwd has no user conversation yet: ${cwd}. It looks like a freshly spawned/bootstrap session you have not engaged with. Did you mean a different session (another runtime, a sister worktree, or a specific session id)?`;
}
function renderCandidateList(candidates) {
  return candidates.map((c) => {
    const identityLabel = c.runtime !== "codex" ? "" : c.parentSessionId || c.nativeSessionId && c.rootSessionId && c.nativeSessionId !== c.rootSessionId ? ` child-of=${c.parentSessionId ?? c.rootSessionId}` : " root";
    return `  ${c.runtime}:${c.sessionId}${identityLabel}  ${c.engagementStatus ?? "unknown"}  records=${c.recordCount ?? "?"}  ${c.transcriptPath}`;
  }).join("\n");
}
function parsePinnedSession2(session) {
  if (!session) return null;
  const colonIndex = session.indexOf(":");
  if (colonIndex === -1) {
    return {
  ...
  const runtime = session.slice(0, colonIndex);
  const sessionId = session.slice(colonIndex + 1);
  if (!isRuntime2(runtime)) {
    return {
      error: `Unknown runtime in --session: ${runtime}. Use one of: ${VALID_RUNTIME_LABEL2}.`
    };
  }
  return { runtime, sessionId };
}
async function applySnippetFilter2(candidates, snippet) {
  if (!snippet) return { candidates, matches: [] };
  const needle = snippet.toLowerCase();
  const matches = [];
  for (const candidate of candidates) {
    let raw;
    try {
      raw = await readFile7(candidate.transcriptPath, "utf8");
    } catch {
      continue;
    }
    const index = raw.toLowerCase().indexOf(needle);
...
async function buildCursorReviewDigest(candidate, args, options) {
  ...
  return buildDigest("cursor", identity.canonicalTranscriptPath, {
    ...
    cursorCapturedAt: (/* @__PURE__ */ new Date()).toISOString()
  });
}
...
async function runReview(args) {
  ...
  let { runtime } = args;
  const pinnedSession = parsePinnedSession2(session);
  if (pinnedSession && "error" in pinnedSession)
    return emitError(pinnedSession.error, 1);
  if (pinnedSession) runtime = pinnedSession.runtime;
  if (runtime === "auto") {
    const resolved = await resolveAutoRuntime2(cwd);
    if (resolved.noMatch) {
      const payload = {
        noMatch: true,
        cwd,
  ...
  if (!isRuntime2(runtime)) {
    return emitError(
      `Unknown runtime: ${runtime}. Use one of: ${VALID_RUNTIME_LABEL2}.`,
      1
    );
  }
  if (runtime === "cursor" && markRead2) {
    ...
    return emitCursorResult(args, result, markRead2);
  }
  let candidates;
  try {
    candidates = await discover(runtime, cwd);
...
${renderCandidateList(error.candidates)}`,
    ...
    if (markRead2 && pinnedRuntime !== "cursor") {
    ...
    if (markRead2 && digest2.schemaVersion === 1) {
      try {
        await markRead(pinnedRuntime, pinned.sessionId, {
  ...
  if (snippet) {
    const filtered = await applySnippetFilter2(candidates, snippet);
    candidates = filtered.candidates;
    if (candidates.length === 0) {
      const payload = {
        noMatch: true,
  ...
  if (rankResult.unengagedOnly) {
    ...
    if (json) return emitJson(payload, 3);
    return emit(
      `${unengagedOnlyMessage2(runtime, cwd)}
...
` + [rankResult.winner, ...rankResult.ties].map((c) => `  ${c.runtime}:${c.sessionId}  (${c.transcriptPath})`).join("\n"),
  ...
  if (markRead2 && digest.schemaVersion === 1) {
    try {
      await markRead(runtime, winner.sessionId, {
        lastRecordIndex: digest.range.nextIndex,
        lastTotalRecords: digest.range.totalRecords,
...
async function runLocate(args) {
  ...
  if (runtime === "auto") {
    const allCandidates = [];
    for (const rt of VALID_RUNTIMES2) {
      try {
    ...
    let snippetMatches2 = [];
    if (snippet) {
      const filtered = await applySnippetFilter2(allCandidates, snippet);
    ...
    if (rankResult2.unengagedOnly) {
      ...
      return emit(
        `${unengagedOnlyMessage2("auto", cwd)}
` + renderCandidateList(rankResult2.candidates),
  ...
  Fallbacks: ${rankResult2.fallbacks.length}` + (rankResult2.fallbacks.length > 0 ? `
${renderCandidateList(rankResult2.fallbacks)}` : ""),
      0
    );
  }
  if (!isRuntime2(runtime)) {
...
` + renderCandidateList(rankResult.candidates),
  ...
  Fallbacks: ${rankResult.fallbacks.length}` + (rankResult.fallbacks.length > 0 ? `
  ...
  switch (stateOp) {
    ...
    case "reset": {
      if (args.session) {
        ...
        if (!isRuntime2(sessionRuntime)) {
          return emitError(
            `Unknown runtime in --session: ${sessionRuntime}. Use one of: ${VALID_RUNTIME_LABEL2}.`,
            1
          );
        }
        try {
          await resetBySession(sessionRuntime, sessionId);
          if (json)
            return emitJson(
      ...
      if (!runtime || runtime === "auto") {
        return emitError(
          `--runtime is required for state reset (use one of: ${VALID_RUNTIME_LABEL2}), or use --session <runtime>:<sessionId>`,
          1
        );
      }
      if (!isRuntime2(runtime)) {
      ...
      try {
        const result = await resetByRuntimeWithDiagnostics(runtime);
...
async function emitNoActiveWatcher(args) {
  await clearControlDirective().catch(() => false);
  const payload = {
    active: false,
...
function consumedThrough2(lastRecordIndex) {
...
function isCursorWatchTarget2(target) {
...
async function singleWatcherStatusPayload(active) {
  const now = Date.now();
  const sessionState = await load().catch(() => ({ sessions: {} }));
  ...
  for (const target of active.targets ?? []) {
    if (isCursorWatchTarget2(target)) {
      ...
      targets.push({
        ...
        consumedThrough: consumedThrough2(lastRecordIndex2),
    ...
    targets.push({
      ...
      transcriptRecords,
      lastRecordIndex,
      consumedThrough: consumedThrough2(lastRecordIndex),
      recordsBehind,
      secondsSinceLastEmit,
      secondsSinceLastPoll,
      staleAfterSec,
...
function sameCwd(a, b) {
  if (!a || !b) return false;
  return resolve3(String(a)) === resolve3(String(b));
}
...
async function runWatchCtl(args) {
  ...
  switch (args.watchCtlOp) {
    ...
    case "flush": {
      ...
      const control = await writeControlDirective(
        args.watchCtlOp,
        { pid: watcher.pid }
      );
      const payload = {
    ...
    case "stop": {
      ...
      const watcher = selected.watcher;
      if (!watcher) return emitError("No watcher selected.", 1);
      const control = await writeControlDirective("stop", {
        pid: watcher.pid
      });
      let signaled = false;
      if (watcher.pid) {
