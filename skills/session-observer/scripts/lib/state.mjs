...
function lockPath2(dir) {
  return join2(dir, "state.json.lock");
}
function cursorTransitionLockPath(dir) {
  return join2(dir, "cursor-state-transition.lock");
}
function tmpPath(dir) {
  return join2(dir, `state.json.${process.pid}.tmp`);
}
function bakPath(dir, label) {
  return join2(dir, `state.json.${label}-${Date.now()}-${process.pid}.bak`);
}
...
async function cleanupAbandonedPrivateTokens2(lock) {
  ...
  for (const name of names) {
    ...
    await unlink2(join2(tokens, name)).catch((error) => {
...
async function contenderOwnsTurn2(lock, ownerPath, owner) {
  ...
  for (const name of (await readdir2(owners)).toSorted()) {
    ...
    const path = join2(owners, name);
    let contenderOwner;
    try {
      contenderOwner = await readFile2(path, "utf8");
    } catch (error) {
...
async function readLockGeneration2(lock) {
  try {
    const rawOwner = await readFile2(lock, "utf8");
...
async function releaseLock2(lock, ownership) {
  try {
    if (await readFile2(lock, "utf8") === ownership.owner) {
      await unlink2(lock);
    }
  } catch (error) {
    if (!isErrnoException2(error) || error.code !== "ENOENT") throw error;
...
async function writeMigrationBackup(dir, destination, raw) {
  ...
  let handle;
  try {
    handle = await open2(temporary, "w", 384);
...
async function removeLegacyCursorForMigration(marker, options) {
  ...
  try {
    ...
    const raw = await readFile2(statePath2(dir), "utf8");
