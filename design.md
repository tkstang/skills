...
- The guidance path opts into transcript-level unattributable summaries for Claude
  Code and Codex. Exact metadata and cwd extracted from a bounded prefix remain
  attributable even when the transcript or record stream continues beyond that
  window. A transcript with missing, malformed, unreadable, or unresolvable cwd
  evidence is omitted while fully attributed source candidates remain available, and
  the public result reports only provider, stable reason code, and count. Enumeration,
  aggregate-budget, and deadline failures still fail the request. Preview and prepare
  discover only the provider encoded in the exact qualified key. The paused executor
  and all other locator consumers retain strict all-or-error defaults.
