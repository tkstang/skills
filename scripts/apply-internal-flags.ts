 ...
 */
export async function applyInternalFlags(
  skillsDir = DEFAULT_SKILLS_DIR,
): Promise<{ checked: number; changed: string[] }> {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  main()
    ...
    .catch((error: unknown) => {
      console.error(
        `apply-internal-flags error: ${error instanceof Error ? error.message : String(error)}`,
      );
      process.exitCode = 2;
    });
}
